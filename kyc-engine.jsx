...
export default function KYCEngine() {
  ...
  const runOCR = useCallback(async (row, index) => {
    ...
    if (idUrl) {
      results.id_proof = await ocrDocument(idUrl, isOrg ? "pan_proof" : "id_proof");
    }
    if (relUrl) {
      results.rel_proof = await ocrDocument(relUrl, "relationship_proof");
    }
                ...
                {!selRow.isProcessing && Object.keys(selRow.ocr).length===0 && (
                  <div style={{ fontSize:11, color:"#1e2d45", padding:"4px 0" }}>
                    {(selRow.row.id_proof_url||selRow.row.id_proof_1_url||selRow.row.relationship_proof_url||selRow.row.rel_proof_url)
                      ...
                      : "No document URLs in this row. Add id_proof_url / relationship_proof_url columns to your CSV."}
                  </div>
                )}
                {selRow.ocr.id_proof  && <OcrDocView label="ID Proof"          ocrData={selRow.ocr.id_proof}  />}
                {selRow.ocr.rel_proof && <OcrDocView label="Relationship Proof" ocrData={selRow.ocr.rel_proof} />}
              </div>
